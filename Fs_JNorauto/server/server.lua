...
TriggerEvent('esx_phone:registerNumber', 'norauto', 'alerte norauto', true, true)

TriggerEvent('esx_society:registerSociety', 'norauto', 'norauto', 'society_norauto', 'society_norauto', 'society_norauto', {type = 'public'})
...
AddEventHandler('Ouvre:mechanic', function()
	...
	for i=1, #xPlayers, 1 do
		...
		-- TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Norauto', '~p~Entreprise', 'Le norauto est désormais ~g~Ouvert~s~ !', 'CHAR_MICHAEL', 8)
		TriggerClientEvent('okokNotify:Alert', xPlayers[i], "Entreprise Norauto", "Le norauto est désormais Ouvert !", 5000, 'neutral')
	end
end)
...
AddEventHandler('Ferme:mechanic', function()
	...
	for i=1, #xPlayers, 1 do
		local xPlayer = ESX.GetPlayerFromId(xPlayers[i])
		-- TriggerClientEvent('esx:showAdvancedNotification', xPlayers[i], 'Norauto', '~p~Entreprise', 'Le norauto est désormais ~r~Fermer~s~ !', 'CHAR_MICHAEL', 8)
		TriggerClientEvent('okokNotify:Alert', xPlayers[i], "Entreprise Norauto", "Le norauto est désormais Fermer !", 5000, 'neutral')
	end
end)
...
AddEventHandler('esx_mecanojob:prendreitems', function(itemName, count)
	...
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_norauto', function(inventory)
...
AddEventHandler("mecano:EndMission", function(job)
    ...
    if xPlayer.job.name ~= "norauto" then
    ...
    TriggerEvent('esx_addonaccount:getSharedAccount', 'society_norauto', function(account)
        if account ~= nil then
