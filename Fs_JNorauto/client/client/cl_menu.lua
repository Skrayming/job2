...
local open = false 
local mainMenu8 = RageUI.CreateMenu('', 'Norauto')
local subMenu8 = RageUI.CreateSubMenu(mainMenu8, "", "Annonces")
local subMenu9 = RageUI.CreateSubMenu(mainMenu8, "", "Interaction véhicule")
	...
	if open then 
		...
		while open do 
		   RageUI.IsVisible(mainMenu8,function() 
			...
			RageUI.Button('Commencer une mission Dépaneuse', nil, { Color = {BackgroundColor = { 76, 175, 80, 50}} }, true, {
				onSelected = function()
					StartMecanoMission()
				end,
			})
			...
			RageUI.Button("Annonce [~g~Disponible~s~]", nil, {RightLabel = "→"}, not codesCooldown1, {
				onSelected = function()
					codesCooldown1 = true 
					TriggerServerEvent('Ouvre:mechanic')
			...
			RageUI.Button("Annonce [~r~Fermetures~s~]", nil, {RightLabel = "→"}, not codesCooldown1, {
				onSelected = function()
					codesCooldown1 = true 
					TriggerServerEvent('Ferme:mechanic')
					Citizen.SetTimeout(60000, function() codesCooldown1 = false end)
		   ...
		   RageUI.IsVisible(subMenu9,function() 
			...
			RageUI.Button("Réparer le véhicule", nil, {RightLabel = "→→"}, true, {
				...
				local towmodel = GetHashKey('f550rbc')
				...
				if isVehicleTow then
					...
					ESX.ShowNotification(_U('imp_f550rbc'))
                end
            end,})
    ...
    if isInMission then
        ...
        Citizen.CreateThread(function()
            ...
            while not IsEntityAttached(veh) and isInMission do
                Visual.Subtitle("Attacher le véhicule sur le f550rbc", 500)
                Wait(500)
            end
